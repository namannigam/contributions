public class Clwyfd {
    public static void main() {
        System.out.println("gvfva");
        System.out.println("vjipc");
        System.out.println("pdcld");
        System.out.println("ftqri");
        System.out.println("qxnwr");
        System.out.println("eeofg");
        System.out.println("gltlw");
        System.out.println("tdeip");
        System.out.println("phiss");
        System.out.println("xhbvk");
        System.out.println("yfubn");
        System.out.println("yysys");
        System.out.println("fuytu");
        System.out.println("xvdrg");
    }
}
